...
double pointToPlaneDistance(double A, double B, double C, double D, double x1,
                            double y1, double z1);
double pointToPlaneDistance(double A, double B, double C, double D, double x1,
                            double y1, double z1) {
...
double compute_distance_with_a_triangle_and_ray(ray R, triangle tri) {
  ...
  return pointToPlaneDistance(
      normal.x, normal.y, normal.z, D, R.ray_current_coordinates.x,
      R.ray_current_coordinates.y, R.ray_current_coordinates.z);
}
...
spectrum_of_light compute_the_ray(ray current_ray, surface_array all_surfaces,
                                  ...
                                  Vector3D_array *final_plane, int qq);
spectrum_of_light compute_the_ray(ray current_ray, surface_array all_surfaces,
                                  triangle_array all_faces,
                                  Vector3D light_position, double light_radius,
                                  uint *seed, int max_recursion,
  ...
  for (int a = 0; a < all_faces.total_number_of_triangles; a++) {
    if (does_triangle_and_ray_intersect_correctly(current_ray,
                                                  all_faces.all_triangles[a])) {
  ...
  for (int o = 0; o < spectrum_of_current_ray.number_of_elements; o++) {
    ...
    spectrum_of_current_ray.spectrum[o] =
        current_ray.ray_spectrum.spectrum[o] *
        (all_surfaces.surface_array[surface_index])
            .surface_spectrum.spectrum[o] *
  ...
  for (int o = 0; o < spectrum_of_current_ray2.number_of_elements; o++) {
    ...
    spectrum_of_current_ray2.spectrum[o] =
        current_ray.ray_spectrum.spectrum[o] *
        (all_surfaces.surface_array[surface_index])
            .surface_spectrum.spectrum[o] *
  ...
  spectrum_of_light specular_spectrum =
      compute_the_ray(new_ray_specular, all_surfaces, all_faces, light_position,
                      light_radius, seed, max_recursion, final_plane, qq);
  spectrum_of_light diffusive_spectrum = compute_the_ray(
      new_ray_diffusion, all_surfaces, all_faces, light_position, light_radius,
      seed, max_recursion, final_plane, qq);
...

{
  ...
  for (int i = 0; i < final_plane.number_of_elements; i++) {
    ...
    spectrum_of_light the_new_spectrum = compute_the_ray(
        current_ray, all_surfaces, all_faces, light_position, light_radius,
        &our_random_number, max_recursion, &final_plane, i);
...
int main() {
  ...
  for (int i = 0; i < NUM_PIXELS_X; i++) {
    ...
    for (int j = 0; j < NUM_PIXELS_Y; j++) {
      ...
      RGB output_colour = trace(i, j, 30, 30, NUM_PIXELS_X, NUM_PIXELS_Y,
                                (int)rand() % RAND_MAX, 0.01,
